#!/usr/bin/env bash
# =============================================================================
# GRPO + LoRA | Review Deficiency Classification | Qwen3.5-9B | SGLang | Single GPU
#
...
# 9B model + SGLang + training on a single 80GB GPU is tight; uses conservative
# token budget and FSDP offload.
#
# Usage:
#   cd /home/ROVER-claudecode/projects/verl-rover
#   bash examples/grpo_trainer/run_review_deficiency_qwen35_9b_lora_sglang.sh
#
# Overrides (env vars):
...
# -- 80GB single GPU: 9B model + SGLang + training is tight; reduce token budget & enable offload
...
experiment_name=${EXPERIMENT_NAME:-qwen35_9b_lora_review_deficiency_sglang_$(date +%Y%m%d_%H%M)}
########################### end user-adjustable ###########################
