#!/usr/bin/env bash
# =============================================================================
# GRPO + LoRA | Review Deficiency Classification | Qwen3.5-9B | Single GPU
#
# Fine-tunes Qwen3.5-9B to classify peer reviews into c0–c6 categories.
...
#
# Usage:
#   cd /home/ClaudeCode/projects/verl
#   bash examples/grpo_trainer/run_review_deficiency_qwen35_9b_lora.sh
#
# Overrides (env vars):
...
########################### user-adjustable ###########################
MODEL_PATH=${MODEL_PATH:-/home/LLM/Qwen/Qwen3.5-9B}
NNODES=${NNODES:-1}
NGPUS_PER_NODE=${NGPUS_PER_NODE:-1}

# -- 80GB single GPU: 9B model + vLLM + training is tight; reduce token budget & enable offload
...
experiment_name=${EXPERIMENT_NAME:-qwen35_9b_lora_review_deficiency_$(date +%Y%m%d_%H%M)}
########################### end user-adjustable ###########################
...
EXTRA=(
    "+actor_rollout_ref.rollout.engine_kwargs.vllm.tensor_parallel_size=1"
)

########################### launch ###########################
echo "=============================================================================="
echo " GRPO + LoRA — Review Deficiency Classification"
echo "=============================================================================="
